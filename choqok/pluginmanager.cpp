...
public:
    ...
    {
        plugins = KPluginInfo::fromServices( KServiceTypeTrader::self()->query( QLatin1String( "Choqok/Plugin" ), QLatin1String( "[X-Choqok-Version] == " + QString::number(CHOQOK_VERSION).toLatin1() ) ) );
    }
...
void PluginManager::loadAllPlugins()
{
    ...
    if ( config->hasGroup( QLatin1String( "Plugins" ) ) )
    {
        ...
        for ( ; it2 != end; ++it2 )
        {
            // Protocols are loaded automatically so they aren't always in Plugins group.
