...
YFrog::YFrog(QObject* parent, const QList<QVariant>& )
    ...
    if ( job->error() ) {
        ...
        if( element.tagName() == "rsp" ) {
            ...
            if(element.hasAttribute("stat") )
                ...
                kError()<<"YFrog uploading failed: There isn't any \"stat\" or \"status\" attribute. Buffer:\n" << buffer;
                return;
            }
            if( result == "ok" ) {
